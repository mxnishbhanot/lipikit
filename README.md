...
product name — `%APPDATA%\<product name>` on Windows, `~/.config/<product
name>` on Linux — and a reinstall finds them again. Renaming the app moves
that directory, which is fine while the name is still a placeholder and
nothing is installed in anger.
