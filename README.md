# AI Anywhere (Temporary)

**The product name is not final.** Every user-visible name, tagline, URL and
address comes from `packages/shared/src/config/branding.ts`, and the icons are
plain placeholder geometry (`apps/desktop/build/icon.svg`, regenerated by
`apps/desktop/scripts/generate-icons.mjs`). Renaming the app means editing
that config plus the two name fields in `apps/desktop/electron-builder.yml`,
